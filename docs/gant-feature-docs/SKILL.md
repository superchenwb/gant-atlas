...
6. `main.md` 必须提供全页面功能索引，方便后续代码生成、修复定位和测试文档消费。
7. 复杂按钮功能优先使用 `button-{name}.md` 一体化描述按钮与其打开的弹窗内容。

## 包路径配置

各业务包的代码根目录（`srcRoot`）与文档根目录（`docsRoot`）的对应关系如下：

| 包名 | 代码根目录 | 文档根目录 |
|---|---|---|
| ibom | `packages/ibom/src` | `packages/ibom/docs` |
| ilowcode | `packages/ilowcode/src` | `packages/ilowcode/docs` |
| ip2system | `packages/ip2system/src` | `packages/ip2system/docs` |
| usersystem | `packages/usersystem/src` | `packages/usersystem/docs` |

**默认规则**：未在上表中列出的包，将代码路径中的 `/src` 替换为 `/docs` 即得到文档路径（`packages/<pkg>/src/... -> packages/<pkg>/docs/...`）。
...
| `components/**/*.tsx` | 无 SearchForm + Grid 组合 | 组件文档 |
...
**核心原则：文档目录结构与代码目录结构完全镜像。** 把代码路径中的 `src` 换成 `docs`，路径的其余部分保持不变，就是文档路径。反之亦然。

```
代码路径  packages/ibom/src/  bbom/bbommain/index.tsx
                         ↓ src 换成 docs
文档路径  packages/ibom/docs/ bbom/bbommain/
```
...
| 场景 | 代码路径 | 对应文档路径 |
|---|---|---|
| 页面目录 | `packages/ibom/src/bbom/bbommain/` | `packages/ibom/docs/bbom/bbommain/` |
| 页面文件 | `packages/ibom/src/ebom/ecrmain/index.tsx` | `packages/ibom/docs/ebom/ecrmain/` |
| 组件目录 | `packages/ibom/src/components/bomcombineselect/` | `packages/ibom/docs/components/bomcombineselect/main.md` |
| Hook 文件 | `packages/ibom/src/hooks/bom/useBomSearch.ts` | `packages/ibom/docs/hooks/bom/useBomSearch.md` |
| 工具函数 | `packages/ibom/src/utils/formatBomCode.ts` | `packages/ibom/docs/utils/formatBomCode.md` |

### 文档 → 代码（读取文档后定位代码时用）

| 文档路径 | 对应代码路径 |
|---|---|
| `packages/ibom/docs/bbom/bbommain/` | `packages/ibom/src/bbom/bbommain/` |
| `packages/ibom/docs/components/bomcombineselect/main.md` | `packages/ibom/src/components/bomcombineselect/` |
| `packages/ibom/docs/hooks/bom/useBomSearch.md` | `packages/ibom/src/hooks/bom/useBomSearch.ts` |

### 补充规则

- 输入为代码**文件**时，先归一到其所在目录（如 `index.tsx` → 父目录），再按上述规则映射。
- 组件文档统一输出为目录下的 `main.md`；Hook/工具函数文档输出为与源文件同名的 `.md`（去掉 `.ts`/`.tsx`）。
- 只对 `packages/*/src/**` 下的业务代码建立映射，procomponents 框架组件不建立文档。
