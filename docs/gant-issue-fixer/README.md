...
通用需求修改与 Bug 修复技能，核心特色：**修改代码时同步更新功能清单文档** + **Spec 驱动 + Worktree 隔离修改**。
...
4. **方案确认** → 展示代码 diff + 文档修改计划 + Spec 执行计划
5. **创建 Spec 计划 + Worktree** → 动态生成步骤列表 + 创建隔离分支目录
6. **执行修改与验证** → 在 Worktree 中修改代码 + 质量检查 + 测试验证 + 文档同步
7. **合并提交与清理** → 合并 Worktree 分支 + 清理 + 云效更新(可选)
