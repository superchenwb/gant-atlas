...
- 当上下文 JSON 中包含 `tabs` 字段时，`button-area.md` 必须包含 "Tab 切换" 章节，列出所有 Tab。
- 当上下文 JSON 中包含 `permissions` 字段时，`button-area.md` 必须包含 "权限矩阵" 章节。
- Tab 和权限数据来自扫描器的确定性提取，优先使用原始值，不确定时写 `[待确认]`。
- 不要因为上下文中缺少某项数据就跳过对应章节 — 如果页面确实有 Tab 或权限，即使扫描器未检测到，也应标注 `[待确认]`。
